...
{
    internal sealed class LogicalAndBinding<TLeft, TRight, TResult> : BinaryBinding<TLeft, TRight, TResult>
    {
        static LogicalAndBinding()
        {
        ...
        public LogicalAndBinding(Binding<TLeft> left, Binding<TRight> right, Func<TLeft, TRight, TResult> op)
            : base(left, right, op)
        {
        }
        ...
        {
            return new LogicalAndBinding<TLeft, TRight, TResult>(context.Clone(Left), context.Clone(Right), Op);
        }
        ...
        protected override BindingResult<TResult> Evaluate()
        {
            ...
            return left == False<TLeft>() || right == False<TRight>() ? False<TResult>() : None<TResult>();
        }
    }
}

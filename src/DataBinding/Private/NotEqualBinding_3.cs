...
{
    internal sealed class NotEqualBinding<TLeft, TRight> : BinaryBinding<TLeft, TRight, bool>
    {
        public NotEqualBinding(Binding<TLeft> left, Binding<TRight> right, Func<TLeft, TRight, bool> op)
            : base(left, right, op)
        {
        }
        ...
        protected override BindingResult<bool> Evaluate()
        {
            ...
            {
                return left.HasValue ? Some(Op(left._value, right._value)) : Some(false);
            }
            return Some(true);
        }
    }
